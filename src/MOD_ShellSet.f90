...
character(len=100),dimension(10) :: ScoreFiles
logical,dimension(8) :: ScoreFilesL
character(len=100),dimension(10) :: ShellsFiles
...
if(OData) then
  ...
  do i = 1,size(DataFiles)
    if(.not. FileExist(trim(dir)//trim(DataFiles(i)))) then
      Abort = .True.
...
do i = 1,5 ! Not all are required
  if(.not. FileExist(trim(dir)//trim(ShellsFiles(i)))) then
    Abort = .True.
    print*,"Shells required file not found: ",trim(dir)//trim(ShellsFiles(i))
  end if
end do
...
do i = 1,5 ! Not all are required
  if(.not. FileExist(trim(dir)//trim(ShellsFiles(i)))) then
    Abort = .True.
    print*,"Shells Final required file not found: ",trim(dir)//trim(ShellsFiles(i))
  end if
end do

call readSCORE(ScoreFiles,ScoreFilesL)
do i = 1,2 ! Not all are required
  if(.not. FileExist(trim(dir)//trim(ScoreFiles(i)))) then
    Abort = .True.
...
subroutine ReadGridInput(VarNames,VarValsMin,VarValsMax,NumMods,Cells,Levels,CellNumMods,ModRpt,RptMod,Abort) ! Read GridInput.in
...
integer,intent(out) :: CellNumMods
...
open(unit=1,file='INPUT/GridInput.in')
  ...
  if(.not. Abort) then
    do i = 1,NumVars
      ...
      CellNumMods = CellNumMods*NumMods(i)
      if(mod(NumMods(i),2)==0) ModRpt = .False.
      if(i/=NumVars) read(1,*) ! skip -----
    end do
  end if
close(1)
...
! Read Variable values from ListInput.in in order
  if(.not. Abort) then
    ...
    do j = 1,NumModels
      ...
      if(j==NumModels .and. i/=NumVar) read(1,*) ! to skip ---- in input file
    end do
    if(Abort) exit
    end do
  end if
...
character(len=100),dimension(10),intent(out) :: ScoreFiles
logical,dimension(8),intent(out) :: ScoreFilesL
integer,intent(in),optional :: ModNum
logical,dimension(7),intent(out),optional :: MisRun

character(len=100) :: Grid,Para,Litho
character(len=100) :: Geo,SeaSprd,StresDir,Fslip,SeisCorre,SeisAniso,Plates
logical :: exists,GeoL,SeaSprdL,StresDirL,FslipL,SeisCorreL,SeisAnisoL,PlatesL,LithoL,GM
character(len=6) :: dir
...
read(111,*) Litho
do i = 1,21
  read(111,*) ! Skip 21 lines to read "Outlines of Plates"
end do
read(111,*) Plates
do i = 1,8
  read(111,*) ! Skip 8 lines
end do
read(111,*) Geo
read(111,*) StresDir
read(111,*) Fslip
...
ScoreFiles = (/Grid,Para,Geo,StresDir,Fslip,SeaSprd,SeisCorre,SeisAniso,Plates,Litho/)
ScoreFilesL = (/GeoL,StresDirL,FslipL,SeaSprdL,SeisCorreL,SeisAnisoL,PlatesL,LithoL/)
if(present(MisRun)) then
  ! MisRun = (/GeoL,SeaSprdL .and. StresDirL,StresDirL,FslipL,SeisCorreL,SeisCorreL .and. SeisAnisoL .and. PlatesL, .True./) ! Must match order in misfit vector output of OrbScore, set GM loc to always true
  MisRun = (/GeoL,SeaSprdL,StresDirL,FslipL,SeisCorreL,SeisCorreL .and. SeisAnisoL .and. PlatesL, .False./) ! Must match order in misfit vector output of OrbScore, set GM loc to always true if more than 1 run
  if(count(MisRun)>1) MisRun = (/GeoL,SeaSprdL,StresDirL,FslipL,SeisCorreL,SeisCorreL .and. SeisAnisoL .and. PlatesL, .True./) ! Must match order in misfit vector output of OrbScore, set GM loc to always true if more than 1 run
end if
if((.not. called) .and. present(ModNum)) then
  ...
  do i =1,7
    if((.not. ScoreFilesL(i)) .and. ScoreFiles(i+2) .ne. 'X') then
      ...
      call ModelError(ErrorMsg,ModNum)
    end if
  end do
...
character(len=100) :: Grid,Para,Bound,Plate,PlateB
...
if(Opt == 'List') frmt = "(I0,X,I0," ! ThID, Model Number
if(Opt == 'Grid' .or. Opt == 'Rptd') frmt = "(I0,X,I0,X,I0,X,I0," ! Model Number, ThID, Level, Cell Number
if(Opt == 'MisL') frmt = "("
...
select case(NumMis)
  ...
  case(7)
    frmt = trim(frmt)//"X,F12.5,X,F12.5,X,F12.5,X,F12.5,X,F12.5,X,F12.5,X,F12.5"
  case default
    print*,'ERROR in number of misfits'
end select


if(Opt == 'Rptd') frmt = trim(frmt)//",X,A"
...
subroutine WriteMisfitTarget(AllModels,VarNames,TargMisM,LvlNumMods,Levels,Step,DirName,MisRank,iLevel) ! Write to Models_Lim.txt
...
character(len=3),intent(in) :: MisRank
...
if(.not. Called) then
  ...
  open(1,file=trim(DirName)//'/Models_Lim.txt',status='old',access='append')
end if
...
subroutine WriteOut(Opt,VarNames,msg_tag,msg_src,iLevel,iCell,VarValues,misfits,MisRun,MisRank,Failed) ! Write to Models.txt
...
character(len=3),intent(in),optional :: MisRank
...
if(opt == 'Grid') then
  ...
  if(trim(MisRank) == 'GV') then
    Non0Mis(1) = misfits(1)
    mask(1) = .False.
    outhdr = trim(outhdr)//", GV"
  elseif(trim(MisRank) == 'SSR') then
    Non0Mis(1) = misfits(2)
    mask(2) = .False.
    outhdr = trim(outhdr)//", SSR"
  elseif(trim(MisRank) == 'SD') then
    Non0Mis(1) = misfits(3)
    mask(3) = .False.
    outhdr = trim(outhdr)//", SD"
  elseif(trim(MisRank) == 'FSR') then
    Non0Mis(1) = misfits(4)
    mask(4) = .False.
    outhdr = trim(outhdr)//", FSR"
  elseif(trim(MisRank) == 'SC') then
    Non0Mis(1) = misfits(5)
    mask(5) = .False.
    outhdr = trim(outhdr)//", SC"
  elseif(trim(MisRank) == 'SA') then
    Non0Mis(1) = misfits(6)
    mask(6) = .False.
    outhdr = trim(outhdr)//", SA"
  elseif(trim(MisRank) == 'GM') then
    Non0Mis(1) = misfits(7)
    mask(7) = .False.
    outhdr = trim(outhdr)//", GM"
  end if
...
elseif(present(Failed)) then
  ...
  elseif(Failed == 100001) then! Var check
    if(Opt == 'Grid') then
      write(101,trim(frmt)) msg_tag,msg_src,iLevel,iCell,VarValues,'Failed input variable check - check values'
    elseif(Opt == 'List') then
      write(101,trim(frmt)) msg_tag,msg_src,VarValues,'Failed input variable check - check values'
    end if
  end if
...
subroutine writeRepeated(ModNum,ThID,iLevel,iCell,VarNames,Repeated) ! Write repeated model output to Models.txt

integer,intent(in) :: ModNum,ThID,iLevel,iCell
character(len=10),dimension(:),intent(in) :: VarNames
real*8,dimension(:),intent(in) :: Repeated

character(len=500) :: frmt


call FormatStrings('Rptd',VarNames,1,frmt)

write(101,frmt) ModNum,ThID,iLevel,iCell,Repeated,'Repeated model - check previous results'
...
select case(errnum)
  ...
  case(1)
    ...
    & "-Iter  -> number of iterations of 'main work loop'", &
    ...
    & "-MR  -> misfit ranker used to drive grid search", &
  ...
  case(2)
    print'(A)',"Problem reading ListInput.in or GridInput.in"

  case(3)
    print'(A)',"FatalError.txt file found - check Error directory"

  case(4)
    print'(A)',"All errors are fatal and either ModelError.txt or FatalError.txt file found  - check Error directory"

  case(5)
    print'(A)',"Program initiated with more threads than ever needed"

  case(6)
    print'(A)',"Problem with the grid search driver or misfit convergence type, check flag -MR, -MC or -GM input"

  case(7)
    print'(A)',"One or more required input files were not found"

  case(8)
    print'(A)',"Non-running CLA"

  case(9)
    print'(A)',"Error when reading CLAs, see previous error message"
  
  case(10)
    print'(A)',"iConve & -Iter values are not compatible, see error files in Error directory"  

  case(11)
    print'(A)',"An input file for this iConve value was not entered, see error files in Error directory"

  case default
    print"(A/,A/,A/,A/,A/,A/,A/,A/,A/,A/,A/,A/,A/)","The following list details the error numbers given inside an MPI_Abort program failure:", &
    &   "0  -> Called with -np 1, program must be started with >1 threads", &
    &   "1  -> CLA problem", &
    ...
    &   "10 -> iConve & -Iter values are not compatible", &
    &   "11 -> An input file for this iConve value was not entered"
...
if(FileExist(Filename)) then
  ...
  open(unit=1,file=trim(Filename),status='new',action='write')
    ...
    write(1,'(A/,A/)') 'Test failed with the following message: ',trim(msg)
...
real*8,dimension(1) :: UpVal
integer :: ios


open(unit=101, file='INPUT/UpVar.in', status='old', action='read')
	
do 
  read(101,*,iostat=ios) UpName(1), UpVal(1)
  ...
  if(ios /= 0) then
    write(ErrorMsg,'(A)') "UpVar.in file not found - program will not iterate"
    call NonFatalError(ErrorMsg,1)
    exit
  end if
...
subroutine CloseOutput(ThID,ModNum,prog,ListDIR,rpeat,Iter) ! Close OrbData, Shells, OrbScore output files
...
integer,intent(in),optional :: rpeat,Iter
...
! FEG for ShellsFinal
    elseif(Iter == 0) then
      write(indir,"(A,'/','ThID_',I0,'_ShellsF_input/')") trim(ListDIR),ThID
      write(infile,"(A,'fort_',i0,'.1')") trim(indir),ModNum
      call execute_command_line( "cp "//trim(outfile)//" "//trim(infile) )
	end if
...
! SHELLS needs to copy the generated vEarth file for OrbScore to use as input
  ...
  case("SF")
    ...
    close(24)
! Shells needs to copy the generated vEarth file for OrbScore to use as input
    write(outdir,"(A,'/','ThID_',I0,'_ShellsF_output/')") trim(ListDIR),ThID
    write(indir,"(A,'/','ThID_',I0,'_Score_input/')") trim(ListDIR),ThID
    write(outfile,"(A,'vEarth_',i0,'_',i0,'.22')") trim(outdir),ModNum,rpeat
    write(infile,"(A,'fort_',i0,'.3')") trim(indir),ModNum
    call execute_command_line("cp "//trim(outfile)//" "//trim(infile))
...
subroutine InputSetup(ThID,ModNum,prog,ListDIR,iConve,MisRun,Verbose) ! Copy OrbData, Shells, OrbScore input files

integer,intent(in) :: ThID,ModNum
character(len=2),intent(in) :: prog ! SH=Shells, OD=Data, OS=Score
character(len=100),intent(in) :: ListDIR
logical,dimension(7),intent(inout),optional :: MisRun
logical,intent(in),optional :: Verbose
integer,intent(in),optional :: iConve
...
! iConve & file check
    if(iConve==6 .and. .not. ShellsFilesL(3)) then
      write(ErrorMsg,'(A,I0,A)') "The iConve value ",iConve," expects a defined Torque & Force balance file in InputFiles.in"
  ...
  case("OS")
    ...
    call readSCORE(ScoreFiles,ScoreFilesL,ModNum=ModNum,MisRun=MisRun) ! (/Grid,Para,Geo,StresDir,Fslip,SeaSprd,SeisCorre,SeisAniso,Plates,Litho/)
    ...
    if(ScoreFilesL(8)) call execute_command_line('cp INPUT/'//trim(ScoreFiles(10))  //' '//trim(dir)//'/'//trim(filename)//'36')
...
! iConve & file check
    if((iConve==1 .or. iConve==2 .or. iConve==5) .and. (.not. ShellsFilesFinalL(2))) then
      write(ErrorMsg,'(A,I0,A)') "The iConve value ",iConve," expects a defined Mantle Flow file in InputFiles.in"
      call FatalError(ErrorMsg,ModNum)
      call abort(11)
    end if
    ...
    if(ShellsFilesFinalL(1)) call execute_command_line('cp INPUT/'//trim(ShellsFilesFinal(6)) //' '//trim(dir)//'/'//trim(filename)//'11')
    if(ShellsFilesFinalL(2)) call execute_command_line('cp INPUT/'//trim(ShellsFilesFinal(7)) //' '//trim(dir)//'/'//trim(filename)//'12')
...
subroutine OpenInput(ThID,ModNum,prog,ListDIR,rpeat,Final) ! Open OrbData, Shells, OrbScore input files
...
select case(prog)
  ...
  case("SH")
    ...
    if(rpeat>1) then ! use generated qEarth file as input
    ...
    elseif(rpeat==1 .and. FileExist(trim(dir)//trim(filename)//'13')) then ! use specified qEarth file as input
	  open(unit=13,file=trim(dir)//trim(filename)//'13')
	end if
  ...
  case("OS")
	...
	if(FileExist(trim(dir)//trim(filename)//'36')) open(unit=36,file=trim(dir)//trim(filename)//'36')
    ...
    if(FileExist(trim(dir)//trim(filename)//'33')) then
      ...
      if(present(Final) .and. Final) then
	    write(dir,"(A,'/','ThID_',I0,'_ShellsF_output/')") trim(ListDIR),ThID
	  else
	    write(dir,"(A,'/','ThID_',I0,'_Shells_output/')") trim(ListDIR),ThID
	  end if
  ...
  case("SF")
    ...
    if(rpeat>1) then ! use generated qEarth file as input
      ...
      write(filename,"('qEarth_',I0,'_',I0,'.')") ModNum,rpeat-1
      open(unit=13,file=trim(dir)//trim(filename)//'24')
    elseif(rpeat==1 .and. FileExist(trim(dir)//trim(filename)//'13')) then ! use old qEarth file as input
	  open(unit=13,file=trim(dir)//trim(filename)//'13')
	end if
...
integer,intent(in),optional :: rpeat
logical,intent(in),optional :: MC

character(len=200) :: filename
character(len=100) :: dir
logical,save :: callOD=.False.,callSH=.False.,callOS=.False.,callSF=.False.
character(len=100) :: ScoreFiles(10)
logical :: ScoreFilesL(8)
...
! Table of Scores file when Misfit Convergence used
    if(MC) then
      write(filename,"('Scores_',I0,'.txt')") ModNum
	  if(FileExist(trim(dir)//trim(filename))) then
        open(unit=iUnitOrbDat,file=trim(dir)//trim(filename),status='old',access='append')
      else
	    open(unit=iUnitOrbDat,file=trim(dir)//trim(filename),status='new',action='write')
	  end if
	end if
...
subroutine DriverChk(MisRank,Abort,GM_Types) ! Check that the driving misfit is actually run

character(len=3),intent(in) :: MisRank
character(len=3),dimension(:),intent(in),optional :: GM_Types
logical,intent(out) :: Abort

logical,dimension(7) :: MisRun
logical,dimension(:),allocatable :: GM_Abort
...
if(trim(MisRank) == 'GM') then
  ...
  allocate(GM_Abort(size(GM_Types)))
  GM_Abort = .True.

  do i =1,size(GM_Types)
    
    select case(GM_Types(i))

      case('GV')
        if(MisRun(1)) GM_Abort(i) = .False.

      case('SSR')
        if(MisRun(2)) GM_Abort(i) = .False.

      case('SD')
        if(MisRun(3)) GM_Abort(i) = .False.

      case('FSR')
        if(MisRun(4)) GM_Abort(i) = .False.

      case('SA')
        if(MisRun(6)) GM_Abort(i) = .False.

    end select

  end do

  if(any(GM_Abort)) then
    print*,'1 or more datasets required for the geometric mean calculations does not exist - program will terminate'
  else
	Abort = .False.
  end if
  ...
  select case(MisRank)
    ...
    case default
      print '(2A, /)', 'Unrecognised misfit ranker: ', MisRank
      Abort = .True.
...
subroutine GenerateModels(NumMods,CellNumMods,Grid,Models) ! Calculate models in grid search level

integer,dimension(:),intent(in)   :: NumMods
integer,intent(in) :: CellNumMods
...
do k = size(Grid,2),1,-1 ! k controls accessed variable (starting from final)
  ...
  ModsAfter = (CellNumMods/ModsBefore)/NumMods(i) ! ModsAfter is repeat number
...
subroutine MisfitAnalysis(misfits,Cells,MisRank,Keep) ! Find best models
...
logical,dimension(7),intent(out) :: MisRun
...
real*8 function GeoMean(allmisfits,GM_Types) ! Calculate geometric mean

real*8,dimension(6),intent(in) :: allmisfits
character(len=3),dimension(:),intent(in),optional :: GM_Types
...
if(present(GM_Types)) then
  ...
  do i =1,size(GM_Types)
    ...
    select case(GM_Types(i))
      ...
      case('GV')
        part = part * allmisfits(1)

      case('SSR')
        part = part * allmisfits(2)

      case('SD')
        part = part * allmisfits(3)

      case('FSR')
        part = part * allmisfits(4)

      case('SA')
        part = part * allmisfits(6)

    end select

  end do

  GeoMean = part**(1.0/real(size(GM_Types)))

else

  do i = 1,6
    if(i==5) cycle ! skip SC error which is not a minimum score
    if(allmisfits(i)==0.0) then
      skip = skip+1
	  cycle
    end if
    part = part * allmisfits(i)
  end do

  GeoMean = part**(1.0/(5.0-real(skip)))
...
subroutine ReadCLA(MisConv,MisRank,MisRange,MinIter,MaxIter,DirName,Verbose,AllErrorFatal,InOpt,TargMisM,TargMisK,GM_Types,Abort) ! Read CLAs

integer,intent(out) :: MinIter,MaxIter
logical,intent(out) :: MisConv,Verbose,AllErrorFatal,Abort
real,intent(out) :: MisRange
character(len=3),intent(out) :: MisRank
character(len=100),intent(out) :: DirName
character(len=4),intent(out) :: InOpt
real,intent(out) :: TargMisM,TargMisK
character(len=3),dimension(:),allocatable,intent(out) :: GM_Types

integer :: i,j,CLA_Count,ierr,c1,c2,c3,c4,c5,GM_Num
logical :: exists,GM_Called
...
do while(i<=command_argument_count())
  ...
  elseif(trim(arg) == '-MC') then
    ...
    elseif(trim(arg(:c1-1)) == 'SA') then
      MisRank = 'SA'
	elseif(trim(arg(:c1-1)) == 'GM') then
      MisRank = 'GM'
    else
      print"(A)","No valid option for misfit ranker was entered - program will terminate"
      Abort = .True.
    end if
    read(arg(c1+1:c1+c2-1),'(I4)',iostat=ierr) MinIter
    if(ierr /= 0) then
      print"(A)","Error reading iterations from Misfit convergence input - program will terminate"
      Abort = .True.
    end if	
    read(arg(c1+c2+1:),'(F8.4)',iostat=ierr) MisRange
    if(ierr /= 0) then
      print"(A)","Error reading range from Misfit convergence input - program will terminate"
      Abort = .True.
    end if	
    i = i+2

  elseif(trim(arg) == '-GM') then
    GM_Called = .True.
    MisRank = 'GM'
    call get_command_argument(i+1,arg)
    c1 = index(arg, ',')
	read(arg(:c1-1),'(I)',iostat=ierr) GM_Num
    if(ierr /= 0) then
      print"(A)","Error reading -GM input integer value - program will terminate"
      Abort = .True.	    
	elseif(GM_Num > 5 .or. GM_Num < 2) then
      print"(A)","-GM input integer value should be in range 2-5 (inclusive) - Program will terminate"
      Abort = .True.
    end if		
	allocate(GM_Types(GM_Num))

    if(GM_Num == 2) then
	  c2 = index(arg(c1+1:), ',')
      read(arg(c1+1:c1+c2-1),'(A3)',iostat=ierr) GM_Types(1)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      end if	  
      read(arg(c1+c2+1:),'(A3)',iostat=ierr) GM_Types(2)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      end if		  
    elseif(GM_Num == 3) then
	  c2 = index(arg(c1+1:), ',')
	  c3 = index(arg(c1+c2+1:), ',')
      read(arg(c1+1:c1+c2-1),'(A3)',iostat=ierr) GM_Types(1)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      end if		  
      read(arg(c1+c2+1:c1+c2+c3-1),'(A3)',iostat=ierr) GM_Types(2)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      end if		  
      read(arg(c1+c2+c3+1:),'(A3)',iostat=ierr) GM_Types(3)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      end if		  
    elseif(GM_Num == 4) then
	  c2 = index(arg(c1+1:), ',')
	  c3 = index(arg(c1+c2+1:), ',')
	  c4 = index(arg(c1+c2+c3+1:), ',')
      read(arg(c1+1:c1+c2-1),'(A3)',iostat=ierr) GM_Types(1)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      ...
      read(arg(c1+c2+c3+1:c1+c2+c3+c4-1),'(A3)',iostat=ierr) GM_Types(3)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      end if		  
      read(arg(c1+c2+c3+c4+1:),'(A3)',iostat=ierr) GM_Types(4)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      end if		  
    elseif(GM_Num == 5) then
	  c2 = index(arg(c1+1:), ',')
	  c3 = index(arg(c1+c2+1:), ',')
	  c4 = index(arg(c1+c2+c3+1:), ',')
	  c5 = index(arg(c1+c2+c3+c4+1:), ',')
      read(arg(c1+1:c1+c2-1),'(A3)',iostat=ierr) GM_Types(1)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      ...
      read(arg(c1+c2+c3+c4+1:c1+c2+c3+c4+c5-1),'(A3)',iostat=ierr) GM_Types(4)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      end if		  
      read(arg(c1+c2+c3+c4+c5+1:),'(A3)',iostat=ierr) GM_Types(5)
      if(ierr /= 0) then
        print"(A)","Error reading -GM inputs - program will terminate"
        Abort = .True.
      end if	  
    else
      print"(A)","Geometric mean expects an integer (2-5) and then a list of Misfit options - program will terminate"
      Abort = .True.	
	end if
	i = i+2
! Check GM inputs	
    do j = 1,GM_Num
	  if(trim(GM_Types(j)) == 'SC') then
	     print"(A)","Seismic correlation should not be used in geometric mean - program will terminate"
	     Abort = .True.
		 exit
      elseif(trim(GM_Types(j)) /= 'GV' .and. trim(GM_Types(j)) /= 'SSR' .and. trim(GM_Types(j)) /= 'SD' .and. &
       & trim(GM_Types(j)) /= 'FSR' .and. trim(GM_Types(j)) /= 'SA') then
	     print"(A,I0)","At least one Geometric mean Misfit option incorrect - program will terminate: ",j
	     Abort = .True.
		 exit
	  end if
    end do	
...
if(MisConv) then ! MisConv requires: MisRank, MisRange, MinIter
  if((MisRank /= 'GV'  .and. MisRank /= 'SSR' .and. MisRank /= 'SD' .and. MisRank /= 'FSR' .and. &
  &  MisRank /= 'SC' .and. MisRank /= 'SA' .and. MisRank /= 'GM')  .or. MisRange == 0.0 .or. MinIter == 0) then
    print'(A)',"Not everything required for Misfit convergence was entered - program will terminate"
    Abort = .True.
  end if
   if(MinIter >= MaxIter) then ! no convergence test necessary
    print'(A)',"-MC iteration is >= -Iter value, this does not make sense - program will terminate"
    Abort = .True.
  end if 
end if
if(MisRank == 'GM' .and. .not. GM_Called) then
  print'(A)',"Program expected -GM CLA input - program will terminate"
  Abort = .True.
end if

! Force Geometric Mean option if -GM CLA entered
if(GM_Called) MisRank = 'GM'
...
print"(/A,A/)","The program requires certain input in the form of command line arguments (CLAs), ",&
   & "an explanaition of the CLAs follows:"

print"(A/,A/,A/)","The following CLAs print information to the terminal without running the program:", &
   & "-help  -> print information currently shown", &
   & "-info  -> print general information about the program"

print"(A/,A/,A/)","The following CLAs are always required for the program to function:", &
   & "-Iter  -> number of iterations of 'main loop'", &
   & "-InOpt -> either 'List' or 'Grid' to determine model input generation"

print"(A/,A/,A/,A/,A/,A/,A/,A/,A/)","The following CLAs add functionality (and may be required):", &
   & "-MC  -> information for misfit convergence", &
   & "-MR  -> misfit ranker used to drive grid search (required for grid search)", &
   & "-GM  -> use defined geometric mean to drive grid search", &
   & "-Dir -> new working directory", &
   & "-AEF -> make all errors fatal", &
   & "-V   -> create Verbose file containing extra informational output", &
   & "-ML  -> misfit limit beneath which all models are stored in an extra output file", &
...
print"(A,A/)","This program allows the testing of multiple input variables inside the SHELLS simulator ", &
   &    "in parallel using either a list of variable names & values or a grid search option."
...
print"(A,A/)","For more information please check the README file and user guide that should have been ", &
   &    "included within the package, found at https://github.com/JonBMay/ShellSet."
