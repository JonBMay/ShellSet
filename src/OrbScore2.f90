...
real*8,dimension(6),intent(out) :: misfits
...
!      Preview .FEG file to determine array sizes:
  ...
  102  CONTINUE
       ...
       END IF
       IF (numeL > mxEl) THEN
	     write(ErrorMsg,'(A,I8,A)') "ERROR: Increase PARAMETER maxEl to at least ", numEl," and recompile OrbScore2."
		 call FatalError(ErrorMsg,ThID)
       END IF
  ...
  103  CONTINUE
       ...
       nFl = MAX(nFl, 0)
       IF (nFl > mxFEl) THEN
	     write(ErrorMsg,'(A,I8,A)') "ERROR: Increase PARAMETER maxFEl to at least ",nFl," and recompile OrbScore2."
		 call FatalError(ErrorMsg,ThID)
       END IF
       DO 105 i = 1, nFl
            READ (iUnitG, "(A)", IOSTAT = ios) longer_line
            IF (ios /= 0) THEN
			  write(ErrorMsg,'(A)') "ERROR: File not found, file is empty, or file is too short."
			  call FatalError(ErrorMsg,ThID)
            END IF
            CALL Extract_LRi (longer_line, &     ! input
                            & LRi, shorter_line) ! output
  ...
  113      FORMAT (/' Attempting to read table of needed Lithospheric Rheologies from unit', I3/)
           ...
           collect_LRs: DO
               ...
               IF ((i < 1).OR.(i > LRn)) THEN
				 write(ErrorMsg,'(A,I8,A,I8,A/,A)') "ERROR: LR# ",i," is outside the legal range of (1:",LRn,").", &
               ...
               ELSE
				 write(ErrorMsg,'(A,I8)') "ERROR while trying to read 13 REAL*8 values that make up LR# ",i
				 call FatalError(ErrorMsg,ThID)
               END IF
           END DO collect_LRs
           CLOSE (iUnitLR)
           ...
           DO j = 1, numEl
               ...
               IF (.NOT.LR_is_defined(i)) THEN
				 write(ErrorMsg,'(A,I8,A,I8,A)') "ERROR: Continuum element ", j," uses LR# ", i," which has NOT been defined!"
				 call FatalError(ErrorMsg,ThID)
               ELSE
                   LR_is_used(i) = .TRUE.
               END IF
           ...
           IF (nFl > 0) THEN
               DO j = 1, nFl
                   i = fault_LRi(j)
                   IF (.NOT.LR_is_defined(i)) THEN
					 write(ErrorMsg,'(A,I8,A,I8,A)') "ERROR: Fault element ", j," uses LR# ", i," which has NOT been defined!"
					 call FatalError(ErrorMsg,ThID)
                   ELSE
                       LR_is_used(i) = .TRUE.
                   END IF
...
!   Input velocities at node points:
   ...
   10 FORMAT(/' ATTEMPTING TO READ VELOCITIES OF NODES ON UNIT', I3/)
     ...
     &             haveNV, title1, title2, title3, v) ! INTENT(OUT)
      IF (.NOT.haveNV) THEN
	     write(ErrorMsg,'(A,I2)') "ERROR: No nodal velocities found on unit ",iUnitV
		 call FatalError(ErrorMsg,ThID)
      END IF
!----------------------------------------------------------------
...
!             ******   SUMMARY OF SCORES ********
  ...
  900 IF(Verbose) then
	  ...
	  END IF
		...
		misfits = (/GeoMisfit,SprdMisfit,StrsMisfit,SlpMisfit,SeisMisfit,AniMisfit/)
