...
subroutine Shells_v5p0(VarNames,VarValues,rpeat,SHELLSconv)
...
!    Obtain extra input file with Lithospheric Rheologies from the user:
       IF (LRn > 0) THEN
           ...
           IF(Verbose) WRITE(iUnitVerb, '(A,I8,A)') 'Lithospheric Rheology indeces from 0 to ', LRn, ' are used in this .feg file.'
           IF(Verbose) WRITE(iUnitVerb, '(A,I3)') 'Attempting to read table of needed Lithospheric Rheologies from unit',iUnitLR

           READ (iUnitLR, * , IOSTAT = ios) ! READ (and discard) column-header line at top
           IF (ios /= 0) THEN
               ErrorMsg = "Shells ERROR: LR File not found, or file is empty or file is too short"
			   call FatalError(ErrorMsg,ThID)
           END IF
           collect_LRs: DO
               ...
               IF ((i < 1).OR.(i > LRn)) THEN
				   WRITE(ErrorMsg,'(A,I0,A,I0,A)') "Shells ERROR: LR# ", i," is outside the legal range of (1:", LRn," To make it legal, some element in the .feg file must use this (or higher) LR#"
				   call FatalError(ErrorMsg,ThID)
               END IF
               BACKSPACE(iUnitLR)
               ...
               ELSE
				   WRITE(ErrorMsg,'(A,I0)') "Shells ERROR: while trying to read 13 REAL*8 values that make up LR# ",i
				   call FatalError(ErrorMsg,ThID)
               END IF
           END DO collect_LRs
           CLOSE (iUnitLR)
           ...
           DO j = 1, numEl
               ...
               IF (.NOT.LR_is_defined(i)) THEN
				   WRITE(ErrorMsg,'(A,I0,A,I0,A)') "Shells ERROR: Continuum element ", j," uses LR# ", i," which has NOT been defined!"
				   call FatalError(ErrorMsg,ThID)
               ELSE
                   LR_is_used(i) = .TRUE.
               END IF
           ...
           IF (nFl > 0) THEN
               DO j = 1, nFl
                   i = fault_LRi(j)
                   IF (.NOT.LR_is_defined(i)) THEN
                       WRITE(ErrorMsg,'(A,I0,A,I0,A)') "Shells ERROR: Fault element ", j," uses LR# ", i," which has NOT been defined!"
					   call FatalError(ErrorMsg,ThID)
                   ELSE
                       LR_is_used(i) = .TRUE.
                   END IF
...
!   Assign each node of grid to a plate:
     ...
     &              checkN)                                   ! work
       ...
       if(rpeat>1 .and. iConve==0) iConve = 6  ! after first iteration use generated qEarth file from previous
       if(rpeat>1 .and. (iConve/=0 .and. iConve/=6)) then
         ErrorMsg = "Shells: iConve value should not be iterated"
         call FatalError(ErrorMsg,ThID)  
       end if
