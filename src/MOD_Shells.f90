...
100       FORMAT (' Attempting to read plate OUTLINES from unit ',I3/)
	DO 140 jEast = 1, 36
		 DO 130 iSouth = -8, 8
			  ...
			  GO TO 103
!                     -------------------- ERR0R HANDLER ----------
101  write(ErrorMsg,'(A/,A,I0/,A,I0,A,I0,A)') "ERR0R IN -Convec-:", &
&                        "WHILE READING MANTLE VELOCITIES FROM UNIT ",iUnitM, &
&                        "TO FILL IN COLUMN ",jEast,", ROW ",iSouth ," ENCOUNTERED A RECORD WHICH DOES NOT HOLD TWO RECOGNIZABLE NUMBERS."
     call FatalError(ErrorMsg,ThID)
!                     ---------------------------------------------
103                 jc = (eLon1 / 10.0D0) + 0.50D0
			  ...
			  IF ((jc /= jEast).OR.(ir /= iSouth)) THEN
                 write(ErrorMsg,'(A,I0/,A,I0,A,I0/,A,I0,A,I0/,A,F7.2,A,F6.2)') "ERR0R: WHILE READING LOWER-MANTLE FLOW VECTORS FROM UNIT ",iUnitM, &
...
10 CONTINUE
11 IF (jPlate == 0) THEN
  write(ErrorMsg,'(A,A2,A/,A)') "Shells ERROR: plate name (",namTag,") not found in lists.", &
...
201 IF(nRead < nPlate) THEN
	    write(ErrorMsg,'(A,I3,A,I3)') "Shells ERROR: Expecting ",nPlate," plates but read outlines of only ",nRead
		call FatalError(ErrorMsg,ThID)
    END IF
...
IF (info /= 0) THEN
  write(ErrorMsg,'(A,I12)') "Shells ERROR: dgbsv (of the MKL library) reports results info = ",info
  call FatalError(ErrorMsg,ThID)
END IF
