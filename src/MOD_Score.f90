...
!      Read the column headers in line 3
   ...
   10  READ (iUnitB, gpsFMT, END = 100) eLonDg, nLatDg, vEMmpa, vNMmpa, &
     &                                  vESigm, vNSigm, correl, frame, tag
	     ...
	     write(ErrorMsg,'(A,I0,A)') "OrbScore ERROR: Standard deviation of velocity is <= 0.0 for benchmark ",numGeo," All values must be positive."
		 call FatalError(ErrorMsg,ThID)
            END IF
            geoTag(numGeo) = TRIM(tag)
       IF (numGeo < mxGeo) GO TO 10
	     write(ErrorMsg,'(A,I0,A)') "SUBPROGRAM GetGEO READ ONLY ",mxGeo," BENCHMARKS. INCREASE PARAMETER maxGeo AND RECOMPILE."
		 call FatalError(ErrorMsg,ThID)
  100  RETURN
...
!   IF FIRST READ FAILS (NO FILE CONNECTED), RETURNS numMOR=0
   ...
   11       FORMAT (A5, F7.2, F8.2, F6.1, F5.1)
       ...
       IF (numMOR < mxMOR) GO TO 10
	     write(ErrorMsg,'(A,I0,A)') "SUBPROGRAM GetMOR READ ONLY ",mxMOR," SPREADING RATES. INCREASE PARAMETER maxMOR AND RECOMPILE."
		 call FatalError(ErrorMsg,ThID)
  100  RETURN
...
!------------------------------------------------------
  ...
  201  IF (nRead < nPlate) THEN
	     write(ErrorMsg,'(A,I3,A,I3)') "OrbScore ERROR: Expecting ",nPlate," plates but read outlines of only ",nRead
		 call FatalError(ErrorMsg,ThID)
       END IF
...
!   Main loop:
   ...
   11       FORMAT (A5, 2F9.3, 7X, I3, 17X, A1, 5X, A2)
            IF ((azimut.LT.-180.0D0).OR.(azimut.GT.360.0D0)) THEN
	           write(ErrorMsg,'(A,I6/,A,A5,2F9.3,7X,I3,17X,A1,5X,A2)') "OrbScore ERROR: Bad azimuth in stress data: ",azimut , &
